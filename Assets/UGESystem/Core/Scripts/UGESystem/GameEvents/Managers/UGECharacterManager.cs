...
namespace UGESystem
{
    ...
    public class UGECharacterManager : MonoBehaviour
    {
        ...
        private Dictionary<string, InteractableObject> _interactableCache = new Dictionary<string, InteractableObject>();
        ...
        public void UpdateCharacterData(CharacterUpdateCommand command)
        {
            ...
            switch (command.UpdateType)
            {
                ...
                case CharacterUpdateType.Partial:
                    if (!string.IsNullOrEmpty(command.OverrideName))
                    {
                        // Maintain the existing SourceTemplateID during partial update
                        targetData.UpdateData(command.OverrideName, targetData.Is3D, targetData.Prefab, targetData.Expressions, targetData.SourceTemplateID);
                    }
                    break;
                case CharacterUpdateType.ResetToOriginal:
                    ...
                    {
                        // Reset to original means SourceTemplateID becomes empty/original
                        RuntimeCharacterDB.UpdateCharacter(command.TargetCharacterId, originalData);
                        // Explicitly clear source template ID if it was reset to original asset
                        var target = RuntimeCharacterDB.GetCharacterData(command.TargetCharacterId);
                        if (target != null)
                        {
                            target.UpdateData(target.Name, target.Is3D, target.Prefab, target.Expressions, "");
                        }
                    }
                    break;
            }
        ...
        /// <summary>
        /// Finds and returns the InteractableObject associated with a given Character ID.
        /// /// (Korean) 주어진 캐릭터 ID와 연동된 InteractableObject를 찾아 반환합니다.
        /// </summary>
        public InteractableObject GetInteractableByCharacterId(string characterId)
        {
            if (string.IsNullOrEmpty(characterId)) return null;

            // Return from cache if available
            if (_interactableCache.TryGetValue(characterId, out var cachedObj) && cachedObj != null)
            {
                return cachedObj;
            }

            // Find in scene if not in cache (Slow, but only happens once per character)
            var interactables = FindObjectsByType<InteractableObject>(FindObjectsSortMode.None);
            foreach (var inter in interactables)
            {
                if (inter.CharacterID == characterId)
                {
                    _interactableCache[characterId] = inter;
                    return inter;
                }
            }
        ...
        /// <summary>
        /// Clears the cache of interactable objects.
        /// </summary>
        public void ClearInteractableCache()
        {
