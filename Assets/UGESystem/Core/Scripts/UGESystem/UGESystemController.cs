...
namespace UGESystem
{
    ...
    public class UGESystemController : Singleton<UGESystemController>
    {
        ...
        /// <summary>
        /// Gets or sets a value indicating whether the system is currently in an interaction state (e.g., bubble chat).
        /// 시스템이 현재 상호작용 상태(예: 버블 챗)인지 여부를 가져오거나 설정합니다.
        /// </summary>
        ...
        // --- Reward Management ---
        private HashSet<string> _grantedRewardIds = new HashSet<string>();
        ...
        #region Reward API
        public bool IsRewardAlreadyGranted(string rewardId)
        {
            if (string.IsNullOrEmpty(rewardId)) return false;
            return _grantedRewardIds.Contains(rewardId);
        }

        public void RegisterGrantedReward(string rewardId)
        {
            if (string.IsNullOrEmpty(rewardId)) return;
            if (!_grantedRewardIds.Contains(rewardId))
            {
            ...
            if (CharacterManager != null && CharacterManager.RuntimeCharacterDB != null)
            {
                foreach (var charData in CharacterManager.RuntimeCharacterDB.Characters)
                {
                    systemState.CharacterStates.Add(new CharacterStateDto
                    {
                        ...
                        Name = charData.Name,
                        SourceTemplateID = charData.SourceTemplateID
                    });
                }
            }

            // 3. Capture Granted Rewards
            systemState.GrantedRewardIds = _grantedRewardIds.ToList();
        ...
        public void RestoreSystemState(UGESystemStateDto savedState)
        {
            ...
            if (CharacterManager != null && CharacterManager.RuntimeCharacterDB != null)
            {
                foreach (var charState in savedState.CharacterStates)
                {
                    ...
                    if (target != null)
                    {
                        // Restore Visuals/Template if a SourceTemplateID was saved
                        if (!string.IsNullOrEmpty(charState.SourceTemplateID))
                        {
                            var template = CharacterManager.RuntimeCharacterDB.GetCharacterData(charState.SourceTemplateID);
                            if (template != null)
                            {
                                // Perform a full restore from template data
                                target.UpdateData(charState.Name, template.Is3D, template.Prefab, template.Expressions, charState.SourceTemplateID);
                            }
                            else
                            {
                                // Fallback: just restore name if template not found
                                target.UpdateData(charState.Name, target.Is3D, target.Prefab, target.Expressions, charState.SourceTemplateID);
                            }
                        }
                        else
                        {
                            // Standard restore (just name)
            ...
            // 2. Restore Reward Registry
            _grantedRewardIds.Clear();
            if (savedState.GrantedRewardIds != null)
            {
                foreach (var id in savedState.GrantedRewardIds)
                {
                    _grantedRewardIds.Add(id);
                }
            }

            // 3. Restore Storyboard Progress
